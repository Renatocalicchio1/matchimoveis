...
- **Pendente, próximos passos nessa ordem**: (1) subir o Listmonk de fato (hoje só Postfix+OpenDKIM estão rodando) — ver `docker-compose.yml`; (2) rodar o `warmup-schedule.js` (rampa de ~18 dias, começa com poucas centenas/dia) antes de disparar pra base completa dos 118k — IP novo mandando volume alto de cara é receita certa pra cair em blacklist; (3) reaplicar o fix do rsyslog na VPS (item acima, só no repo por enquanto).
