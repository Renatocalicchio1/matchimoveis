...
app.get('/', (req,res)=>{
  res.render('index', { flash: null, result: { stats: {}, filters: { cities: [], neighborhoods: [], bairros: [], status: [], scores: [], matchStatuses: [{ value: 'all', label: 'Todos' }, { value: 'with-match', label: 'Com match' }, { value: 'without-match', label: 'Sem match' }] }, results: [], duplicates: [] } });
});
...
app.get('/app-imoveis', (req,res)=>res.redirect('/admin/match'));
...
app.get('/app-leads', (req,res)=>{
  ...
  res.render('app-leads', { user: usuario, leads });
});
