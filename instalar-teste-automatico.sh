...
echo "🔧 Corrigindo NLP e cobertura..."

# ── FIX DIRETO no nlp.js — expandir detectarDominio ──────────────────────────
node << 'EOF'
const fs = require('fs');
let f = fs.readFileSync('cerebro/nlp.js','utf8');

// Substituir função detectarDominio completa
const nova = `function detectarDominio(mNorm) {
...
f = f.replace(/function detectarDominio\(mNorm\) \{[\s\S]*?\n\}/m, nova);
fs.writeFileSync('cerebro/nlp.js', f);
console.log('✅ detectarDominio expandido');
EOF

# ── CRIAR teste-automatico.js ─────────────────────────────────────────────────
cat > teste-automatico.js << 'JSEOF'
'use strict';
/**
...
echo "✅ teste-automatico.js criado!"

# Adicionar script no package.json
node -e "
const fs=require('fs');
const pkg=JSON.parse(fs.readFileSync('package.json','utf8'));
pkg.scripts['teste']='node teste-automatico.js';
fs.writeFileSync('package.json',JSON.stringify(pkg,null,2));
console.log('✅ npm run teste adicionado');
"
