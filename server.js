...
app.get('/cliente/visita/:id/confirmar', async (req, res) => {
  try {
    const { query: _qCG } = require('./services/db');
    await _qCG("UPDATE visitas SET status='lead_confirmou', confirmacao_cliente_status='CONFIRMADO', confirmacao_cliente_em=NOW() WHERE id=$1", [req.params.id]);
    const _vRow = await _qCG('SELECT * FROM visitas WHERE id=$1', [req.params.id]);
