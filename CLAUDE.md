...
- **Pendente, nessa ordem, antes do 1º disparo real pros 118k**:
  1. Testar o SMTP de verdade (enviar 1 email de teste pelo próprio painel do Listmonk, não só confiar na config salva)
  2. Configurar limite de envio (Settings → Performance) — sem isso o Listmonk pode mandar o lote do dia inteiro em rajada, anulando o propósito do aquecimento gradual
  3. Trocar o conteúdo placeholder do `dispararLoteDoWarmup()` (`services/listmonkSync.js`) — hoje manda assunto/corpo fixo de teste ("Novidades no MatchImóveis 🏠" / "Conteúdo da campanha — edita no painel..."), não pode ir pra corretor real assim
  4. Rodar um teste pequeno (2-3 emails de teste, não a base real) pra validar `sincronizarContatosListmonk()` e `dispararLoteDoWarmup()` contra essa instância de verdade — nenhum dos dois nunca rodou contra um Listmonk real, foram escritos sem essa validação
  5. Só depois disso, criar um **Cron Job no Render** (novo serviço, mesmo repositório, `node infra/email-marketing/warmup-schedule.js`, 1x/dia) com env vars `DATABASE_URL` (copiado do serviço `matchimoveis` web), `LISTMONK_URL=http://179.197.225.121:9000`, `LISTMONK_API_USER`, `LISTMONK_API_TOKEN` — **por que Cron Job no Render e não cron na própria VPS**: `warmup-schedule.js` faz `require('../../services/listmonkSync')`, ou seja, depende de rodar de dentro de um checkout completo do repo da app (que já tem `DATABASE_URL` configurado), não só da pasta `infra/email-marketing/` isolada que está na VPS.
  6. Reaplicar o fix do rsyslog (item da seção anterior) na VPS — só está no repo por enquanto.
