...
- [ ] **URGENTE — rodar `node criar-indices-pendentes.js` de novo no Render Shell (ago/2026)**: script atualizado com 4 índices novos (`idx_leads_atualizado_em`, `idx_imoveis_atualizado_em`, `idx_usuarios_atualizado_em`, `idx_visitas_atualizado_em`) — causa raiz encontrada pro servidor caindo o dia todo mesmo depois dos fixes de pool/jobs anteriores: os caches incrementais de leads/imoveis/usuarios/visitas rodam pra sempre em loop (15-30s) fazendo `WHERE atualizado_em > $1` sem índice nessa coluna — Seq Scan na tabela inteira a cada disparo, piorando conforme a base cresce (uma conta já passa de 8.000 imóveis). 4 desses rodando em paralelo esgotam o pool de 15 conexões, tudo mais (sessão, jobs, `/health`) fica sem conexão, Render mata o processo (SIGTERM) achando que travou. `IF NOT EXISTS` no script pula rápido os 8 índices já criados antes, só cria os 4 novos. Enquanto não rodar, o padrão de queda "sistema não abre, fica girando" tende a se repetir.
- [ ] **Assinatura SNS do webhook de bounce/reclamação (ago/2026) presa em "Confirmação pendente"** — código já no ar (`/webhook/ses-notificacoes`, `services/sesWebhook.js`, commit `6c5b036e`), revisado de novo (ago/2026): assinatura RSA, parse de `SubscriptionConfirmation`/`Notification`, tudo confere — não há bug de código a corrigir aqui. Tentativa de bater no endpoint de produção direto daqui (`curl https://www.matchimoveis.ia.br/webhook/ses-notificacoes` e `/health`) foi bloqueada pelo proxy de saída do próprio sandbox (`CONNECT tunnel failed, 403`) — não prova nada sobre o servidor, é limitação do ambiente de execução, não repetir esse teste a partir daqui. Segue precisando de ação no console: Renato checar no painel do Render se o deploy mais recente está "Live" e se é plano free (cold start atrasando a confirmação). Se confirmar deploy ok: apagar a subscription pendente no SNS e criar de novo (força nova tentativa de confirmação). Enquanto não confirmar, bounce/reclamação **não está sendo suprimido automaticamente** — só o opt-out manual funciona. Antes de aumentar ainda mais o volume de disparo da campanha geral (ver item de intervalo abaixo), o Renato pediu pra revisitar essa pendência primeiro — ainda não foi retomado.
- [ ] **`check-usuarios-compraram.js` criado (ago/2026), o Renato nunca rodou/reportou o resultado** — script pronto em `check-usuarios-compraram.js` (Render Shell: `node check-usuarios-compraram.js`) lista quem comprou de verdade via Mercado Pago (`combo_leads_promo`/`combo_demanda`/`recarga_mp` em `dados.matchCoinsTransacoes`, excluindo bônus). Pergunta original "quem comprou?" segue sem resposta real até rodar.
...
- [ ] **Comissão de sub-admin: sem notificação proativa quando é paga (ago/2026)** — hoje o sub-admin só vê que foi pago quando abre `/admin/minhas-comissoes` (card "Pagamentos recebidos" + planilha com data). Não existe aviso automático (WhatsApp/e-mail) disparado no momento em que o superadmin marca como pago em `/admin/comissoes-pendentes` — foi oferecido ao Renato, ele não confirmou se quer.
