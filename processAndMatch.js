...
(async () => {
  ...
  for (const r of rows) {
    ...
    console.log('\n============================');
    console.log('LEAD:', lead.id, lead.nome);

    console.log('EXTRAINDO:', lead.url);
    let origin = {};
    try {
    ...
    console.log('EXTRAÍDO:', lead.bairro, lead.tipo, lead.valor_imovel, lead.area_m2, lead.quartos);

    if (lead.bairro && lead.tipo && lead.quartos) {
      console.log('MATCH BUSCANDO:', lead.bairro, lead.tipo, lead.quartos);
      try {
        const candidatos = await searchQuintoAndar(lead);
        const matches = findTopMatches(lead, candidatos);
    ...
    console.log('SALVO:', lead.id);
  }

  console.log('\n============================');
  console.log('TOTAL NA BASE:', data.length);
  console.log('ADICIONADOS:', adicionados);
  console.log('DUPLICADOS:', duplicados);
  console.log('LEADS NOVOS COM MATCH:', comMatch);
})();
