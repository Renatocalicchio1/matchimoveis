...
app.get('/app/imoveis', auth, (req,res)=>{
  const imoveis = filtrarPorUsuario(loadImoveis(), req.session.user);
...
app.get('/app/leads', auth, (req,res)=>{
  ...
  res.render('app-leads', { user, leads, stats });
});
...
function filtrarPorUsuario(lista, user){
  ...
  const tel = String(user.celular || user.telefone || '');
