...
app.get('/enviar-lead/:id', async (req, res) => {
  try {
    ...
    if (!item || !item.origin) {
      return res.send('Lead não encontrado');
    }

    const matches = item.matches || [];

    let mensagem = `🚀 Novo lead com match\n\n`;
    mensagem += `Cliente: ${item.clientName || 'N/A'}\n`;
    mensagem += `Telefone: ${item.phone || 'N/A'}\n`;
    mensagem += `Email: ${item.email || 'N/A'}\n\n`;

    mensagem += `Imóvel buscado:\n`;
    mensagem += `Bairro: ${item.origin.bairro}\n`;
    mensagem += `Valor: ${item.origin.valor}\n`;
    mensagem += `Quartos: ${item.origin.quartos}\n\n`;

    if (matches.length) {
      mensagem += `Matches encontrados:\n`;
    ...
    await sendWhatsApp(process.env.TEST_CORRETOR_WHATSAPP, mensagem);

    res.send('Lead enviado no WhatsApp 🚀');
  } catch (err) {
    console.error(err);
    res.send('Erro ao enviar lead');
  }
});


// 🚀 ENVIO AUTOMÁTICO APÓS MATCH

async function enviarLeadSeTemMatch(item) {
  try {
    if (!item || !item.matches || item.matches.length === 0) return;

    if (item.whatsSent) return; // evita duplicar envio

    const origin = item.origin || {};
    const matches = item.matches || [];

    let msg = `🚀 Lead com match pronto\n\n`;
    ...
    msg += `Matches:\n`;
    matches.slice(0, 5).forEach((m, i) => {
      msg += `${i + 1}) ${m.url || m.cleanUrl || 'link não disponível'}\n`;
    });

    const sid = await sendWhatsApp(process.env.TEST_CORRETOR_WHATSAPP, msg);

    console.log('WHATS ENVIADO:', sid);

    item.whatsSent = true; // marca como enviado

  } catch (err) {
    console.error('Erro ao enviar Whats:', err.message);
  }
}
