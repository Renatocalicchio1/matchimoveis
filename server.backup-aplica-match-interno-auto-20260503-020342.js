...
app.get('/app/lead/:id', auth, (req, res) => {
  ...
  const origemLead = {
    ...
    valor_imovel: Number(lead.valor_imovel || lead.valor || 0),
    area_m2: Number(lead.area_m2 || lead.area || 0),
    quartos: Number(lead.quartos || 0),
    suites: Number(lead.suites || 0),
    banheiros: Number(lead.banheiros || 0),
    vagas: Number(lead.vagas || 0)
  };

  let matchesInternos = [];
  try {
    const { findTopMatches } = require('./services/matcher');
    matchesInternos = findTopMatches(origemLead, imoveisInternos, 8);
  } catch(e) {
    matchesInternos = [];
  }
