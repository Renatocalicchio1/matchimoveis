...
- [ ] **Assinatura SNS do webhook de bounce/reclamação (ago/2026) presa em "Confirmação pendente"** — código já no ar (`/webhook/ses-notificacoes`, `services/sesWebhook.js`, commit `6c5b036e`), tópico SNS `ses-bounces-complaints` criado e ligado em SES → Identidades → matchimoveis.online → Notificações de feedback (Bounce+Complaint apontando pro tópico). Criada a subscription HTTPS pro webhook, mas depois de 5+ min nenhum log apareceu no servidor (`[ses-webhook] ...`) — a requisição de confirmação da AWS não chegou a bater no servidor. Suspeita: timing (assinatura criada bem na hora que o deploy do Render ainda estava reiniciando) ou plano free do Render "dormindo". Renato ia checar no painel do Render se o deploy mais recente está "Live" e se é plano free (cold start). Próximo passo se confirmar deploy ok: apagar a subscription pendente no SNS e criar de novo (força nova tentativa de confirmação). Enquanto não confirmar, bounce/reclamação **não está sendo suprimido automaticamente** — só o opt-out manual funciona.
